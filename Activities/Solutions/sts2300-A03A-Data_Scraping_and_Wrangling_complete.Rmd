...
# Required Section: Scraping Data with `rvest`
...
1. The Hugo Award is given each year for the best science fiction or fantasy novel. You can see the Wikipedia page for the award [here](https://en.wikipedia.org/wiki/Hugo_Award_for_Best_Novel). The code below takes the URL, reads the html code for the page, and extracts all the tables it finds. This is stored in an object called hugo_award. Run the code below and then use the console to see what the `hugo_award` object looks like.
