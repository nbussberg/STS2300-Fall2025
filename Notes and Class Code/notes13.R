...
ggplot(bodytemps) + 
  geom_boxplot(aes(x=temperature), fill = "green") + 
  theme_classic()


# Calculate the sample mean

xbar <- mean(bodytemps$temperature)
xbar
...
visualize(null_temp) + 
  shade_p_value(obs_stat = xbar, direction = "both")

# calculate the p-value

null_temp |> 
get_p_value(obs_stat = xbar, direction = "both")
...
prop.test(x = 95, n = 321+95, 
          p = 0.25, alternative = "less")


## t.test() for µ

t.test(x = bodytemps$temperature, mu = 98.6,
